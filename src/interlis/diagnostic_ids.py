...
REGISTRY: dict[str, tuple[str, str]] = {
    ...
    "SQL-GEOM-NO-CRS": ("C", "geometry column has no resolvable CRS/domain - provide the geometry base model via --repo"),
    "SQL-FK-CROSS-MODEL-DROPPED": ("C", "FOREIGN KEY target lives in a model not in this conversion - pass it via --catalog"),
    # -- convert-sql: UNIQUE -------------------------------------------------
    ...
    "SQL-UNIQUE-LOCAL-UNSUPPORTED": ("A", "UNIQUE (LOCAL) path shape outside the single-role-hop + attribute-list form"),
    "SQL-UNIQUE-COL-UNMAPPED": ("A", "UNIQUE references a column whose attribute type was not mapped"),
    # -- convert-sql: CHECK / CONSTRAINT ----------------------------------
    "SQL-CHECK-EXPR-UNSUPPORTED": ("B", "CONSTRAINT expression needs object-graph/aggregate/function context a per-row CHECK cannot express"),
    "SQL-CONSTRAINT-NOT-ROWLOCAL": ("B", "CONSTRAINT is not a row-local MANDATORY CONSTRAINT - no CHECK generated"),
    "SQL-CONSTRAINT-PLAUSIBILITY": ("B", "percentage-based plausibility CONSTRAINT - a population ratio, not a per-row CHECK"),
    ...
    "SQL-VIEW-BASE-MISSING": ("C", "a VIEW base/join/navigation target table is not in this conversion - pass its model via --repo/--catalog"),
    "SQL-VIEW-EXPR-UNTRANSLATABLE": ("B", "a VIEW attribute/WHERE expression is outside the path/constant/DEFINED/relational subset a SQL view can carry"),
    "SQL-VIEW-ATTR-DROPPED": ("A", "an ALL OF pass-through attribute could not be projected as a single column and was dropped"),
    "SQL-VIEW-NO-ATTRS": ("B", "a VIEW has no projectable ATTRIBUTE definitions - nothing to SELECT"),
    "SQL-VIEW-CONSTRAINT-DROPPED": ("B", "a VIEW-level UNIQUE/SET/EXISTENCE/CONSTRAINT cannot be carried onto a CREATE VIEW"),
    # -- convert (.ili -> JSON Schema) -------------------------------------
    "JSONSCHEMA-CLASS-UNRESOLVED": ("C", "a nested-structure Class is not reachable from the given roots - provide its model via --repo"),

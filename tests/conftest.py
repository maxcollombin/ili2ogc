...
it cost. These three functions are the canonical versions; a test file whose
...
parse/build/warnings boilerplate itself. There is deliberately no
`build_from_text_with_model` counterpart: every caller needing the `model`
return value parses from a fixture file, not from an inline snippet - add
one only once a real caller needs it (a first Phase 1 draft added it
speculatively with zero callers; removed in the Phase 1b review).
